...
function generateWebhookSignature(secret, payload) {
  ...
  return crypto2.createHmac("sha256", secret).update(content, "utf8").digest("hex");
}
function verifyWebhookSignature(secret, payload, signature) {
  ...
  return crypto2.timingSafeEqual(Buffer.from(expected), Buffer.from(signature));
}

export { BaseService, FoodService, GOBIZ_URLS, GlobalPortalPoller, GoAuthService, GoBiz, GoBizError, GoBizPortal, GoBizPortalService, GoBizScope, GoFoodOrderStatus, OutletService, PaymentService, PaymentTransactionStatus, PaymentType, buildDynamicQris, crc16Ccitt, generateWebhookSignature, verifyWebhookSignature };
